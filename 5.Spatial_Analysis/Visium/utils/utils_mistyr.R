...
#' add_paraview(family= "constant"): extract knn instead
#' 

reticulate::use_condaenv("/home/nc3018/conda/my-envs/cell2loc_env")
require(Seurat)
require(ggpubr)
...
extract_seurat_data <- function(visium.slide,
                                assay,
                                geometry) {
  logger::log_info("* Extracting ",assay," ...")
  
  DefaultAssay(visium.slide) = assay
  logger::log_info("* Default assay set to ", DefaultAssay(visium.slide))
  data <- GetAssayData(visium.slide, assay = assay, slot="data") %>%
    as.matrix() %>%
    t() %>%
...
filter_data_features <- function(data,
  ...
  return(data %>% rownames_to_column() %>%
           dplyr::select(rowname, all_of(features%>%intersect(colnames(data)))) %>% rename_with(make.names) %>%
           column_to_rownames())
}
...
create_default_views <- function(data,
  ...
  if (view.type == "intra") {
    ...
    view.data.tmp <- view.data.init %>%
      add_paraview(geometry, l = view.param, family = para.family )
...
build_misty_pipeline <- function(view.data,
  ...
  # Run MISTy
  run_misty(pline.views, out.alias, cached = FALSE, bypass.intra   = F)
}
