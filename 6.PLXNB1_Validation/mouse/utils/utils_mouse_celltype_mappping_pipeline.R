...
plxnb1_ko_mouse_label_transfer_pipeline = function(query.sub.list,query.assay = "Spatial",
                                   reference.sub,
  ...
  log_info("-----------------------------------------------------------------")
  log_info("      PLXNB1 KO MOUSE DATA LABEL TRANSFER               ")
  log_info("-----------------------------------------------------------------")
...
for (s in c(1:length(query.sub.list))){
  ...
  if (length(query.sub.list)>1){
    query = merge(query.sub.list[[1]],query.sub.list[-1],
                  project = "huang et al mouse plxnb1 ko")
  ...
  if (harmony_query & length(query.sub.list)>1){
    ...
    query_harmony = query
    query_harmony@meta.data[["condition"]] = str_replace(query_harmony@meta.data[["orig.ident"]],"-Male-6 Month-Hippocampus","")
  ...
  p2 = Stacked_VlnPlot(seurat_object = query, features = nichenetr::convert_human_to_mouse_symbols(plot.markers),colors_use = pal,group.by = "predicted.state",x_lab_rotate = T)+ ggtitle("Query (Seurat Label Transfer)")
  ...
  p1 = DimPlot_scCustom(reference.sub, reduction = "umap", group.by = label_oi, label = TRUE, label.size = 3.5,
                        repel = TRUE,colors_use = pal) + ggtitle("Reference annotations")

  p2 = DimPlot_scCustom(query, reduction = "ref.umap", group.by = "predicted.state", label = FALSE,pt.size = 0.8,
                        label.size = 4, repel = TRUE,colors_use = pal) + ggtitle("Query transferred labels")

  ggsave(p1+p2,filename = file.path(output.dir, "ref.umaps.pdf"),width = 15, height = 5)

  # Plot Markers expression on UMAP -----
  p = FeaturePlot_scCustom(query,features =nichenetr::convert_human_to_mouse_symbols(plot.markers),reduction = "ref.umap",num_columns = 3)
